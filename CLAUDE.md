...
- 📈 **자동 매도**: 1% 수익률 도달 시 한 틱 아래 가격으로 지정가 매도
...
- `place_limit_sell_order()`: 지정가 매도 주문
...
3. 목표 수익률(1%) 도달 확인
4. 매도가 계산: `목표가 - 1틱`
5. REST API로 지정가 매도 주문 실행
6. 매도 결과를 `trading_results/` 디렉토리에 저장
